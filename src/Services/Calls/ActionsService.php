...
final class ActionsService implements ActionsContract
{
     ...
     * @param ConversationRelayInterruptible|value-of<ConversationRelayInterruptible> $interruptible Controls when caller input can interrupt assistant speech. `any` allows speech or DTMF interruptions; `none` disables interruptions; `speech` allows speech only; `dtmf` allows DTMF only.
     * @param ConversationRelayInterruptible|value-of<ConversationRelayInterruptible> $interruptibleGreeting Controls when caller input can interrupt assistant speech. `any` allows speech or DTMF interruptions; `none` disables interruptions; `speech` allows speech only; `dtmf` allows DTMF only.
    ...
    public function startConversationRelay(
        ...
        ?string $greeting = null,
        ConversationRelayInterruptible|string $interruptible = 'any',
        ConversationRelayInterruptible|string $interruptibleGreeting = 'any',
        ...
        // @phpstan-ignore-next-line argument.type
        $response = $this->raw->startConversationRelay($callControlID, params: $params, requestOptions: $requestOptions);
     ...
     *  C - Krisp
     *  D - AiCoustics
     * @param NoiseSuppressionEngineConfig|NoiseSuppressionEngineConfigShape $noiseSuppressionEngineConfig Configuration parameters for noise suppression engines. Different engines support different parameters.
     * @param RequestOpts|null $requestOptions
     *
     * @throws APIException
     */
    ...
    public function stopConversationRelay(
        ...
        // @phpstan-ignore-next-line argument.type
        $response = $this->raw->stopConversationRelay($callControlID, params: $params, requestOptions: $requestOptions);
     ...
     * @param MediaEncryption|value-of<MediaEncryption> $mediaEncryption Defines whether media should be encrypted on the new call leg. For SIP URI destinations, media encryption can also be requested per endpoint with the `secure` URI parameter: `;secure=true` or `;secure=srtp` enables SRTP, and `;secure=dtls` enables DTLS. This parameter, when set to `SRTP` or `DTLS`, takes precedence over the per-endpoint `secure` value.
