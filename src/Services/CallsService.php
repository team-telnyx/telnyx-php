...
final class CallsService implements CallsContract
{
     ...
     * @param MediaEncryption|value-of<MediaEncryption> $mediaEncryption Defines whether media should be encrypted on the call. For SIP URI destinations, media encryption can also be requested per endpoint with the `secure` URI parameter: `;secure=true` or `;secure=srtp` enables SRTP, and `;secure=dtls` enables DTLS. This parameter, when set to `SRTP` or `DTLS`, takes precedence over the per-endpoint `secure` value.
     ...
     * @param array<string,list<string>> $webhookURLs A map of event types to arrays of webhook URLs. When an event of the specified type occurs, the webhook URLs associated with that event type will be called instead of the default webhook URL. Events not mapped here will use the default webhook URL.
    ...
    public function dial(
        ...
        $params = array_filter(
            [
                ...
                'conferenceConfig' => $conferenceConfig ?? Omitted::VALUE,
                ...
                'superviseCallControlID' => $superviseCallControlID ?? Omitted::VALUE,
